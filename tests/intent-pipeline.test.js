...
const store = { message_logs: [], detection_logs: [], ai_detection_logs: [], validation_logs: [], execution_logs: [], system_errors: [] }
...
async function run() {
  ...
  const ctx = pipeline.createContext('91', 'Netflix ends at 7 PM tomorrow')
  await pipeline.stageNormalize(ctx)
  assert.ok(ctx.messageId)
  assert.equal(store.message_logs.length, 1)

  const intent = await pipeline.stageDetect(ctx, 'Netflix ends at 7 PM tomorrow')
  assert.equal(store.detection_logs.length, 1)
  ...
  assert.equal(store.validation_logs.length, 1)
  assert.equal(v.passed, true)

  await pipeline.stageExecute(ctx, 'SUBSCRIPTION_EXPIRY', async () => ({ ok: true }))
  assert.equal(store.execution_logs.length, 1)
